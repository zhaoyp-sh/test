 ...
 */
var Loader = function (callback, isCache, config) {
    this.that = this;
    // 回掉函数
    this.done = callback;
    // 配置文件
...
Loader.prototype = {
    init: function () {
        var that = this;
        this.load("script", this.config, function () {
            that.loadConfig(
                page_config,
                function (total, index, pindex, real_index, real_total) {
                  ...
                  /*  document.getElementById("loader_data").innerHTML = Math.floor(rate * 100) + "%";*/
                    //document.getElementById("loader_bar").style.backgroundPositionX = -95 + 95 * rate + 'px';
                },
                function (pindex) {
